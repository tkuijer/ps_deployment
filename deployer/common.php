...
task('rollback', function () {
    ...
    if (isset($releases[1])) {
        ...
        run("cd {{deploy_path}} && $sudo ln -nfs $releaseDir current");

        // Remove release
        run("$sudo rm -rf {{deploy_path}}/releases/{$releases[0]}");
...
task('deploy:prepare', function () {
    ...
    // Create releases dir.
    run("cd {{deploy_path}} && if [ ! -d releases ]; then $sudo mkdir releases; fi");

    // Create shared dir.
    run("cd {{deploy_path}} && if [ ! -d shared ]; then $sudo mkdir shared; fi");
...
task('deploy:release', function () {
    ...
    run("$sudo ln -s $releasePath {{deploy_path}}/release");
...
task('deploy:update_code', function () {
    ...
    run("$sudo git clone $at --depth 1 --recursive -q $repository {{release_path}} 2>&1");
...
task('deploy:shared', function () {
    ...
    foreach (get('shared_dirs') as $dir) {
        // Remove from source
        run("if [ -d $(echo {{release_path}}/$dir) ]; then $sudo rm -rf {{release_path}}/$dir; fi");

        // Create shared dir if it does not exist
        run("$sudo mkdir -p $sharedPath/$dir");

        // Create path to shared dir in release dir if it does not exist
        // (symlink will not create the path and will fail otherwise)
        run("$sudo mkdir -p `dirname {{release_path}}/$dir`");

        // Symlink shared dir to release dir
        run("$sudo ln -nfs $sharedPath/$dir {{release_path}}/$dir");
    }

    foreach (get('shared_files') as $file) {
        // Remove from source
        run("if [ -f $(echo {{release_path}}/$file) ]; then $sudo rm -rf {{release_path}}/$file; fi");

        // Create dir of shared file
        run("$sudo mkdir -p $sharedPath/" . dirname($file));

        // Touch shared
        run("$sudo touch $sharedPath/$file");

        // Symlink shared dir to release dir
        run("$sudo ln -nfs $sharedPath/$file {{release_path}}/$file");
    }
...
task('deploy:writable', function () {
    ...
    if (!empty($dirs)) {
        ...
        try {
            ...
            if (strpos(run("sudo chmod 2>&1; true"), '+a') !== false) {
...
task('deploy:vendors', function () {
    ...
    if (commandExist('composer')) {
        ...
        run("cd {{release_path}} && curl -sS https://getcomposer.org/installer | $sudo php");
        $composer = 'php composer.phar';
    }

    run("cd {{release_path}} && {{env_vars}} $sudo $composer {{composer_options}}");
...
task('deploy:symlink', function () {
    $sudo = get('writable_use_sudo') ? 'sudo' : '';
    run("cd {{deploy_path}} && $sudo ln -sfn {{release_path}} current"); // Atomic override symlink.
    run("cd {{deploy_path}} && $sudo rm release"); // Remove release link.
...
task('deploy:chown', function () {
    $user = get('chown_user');
    $group = get('chown_group');
    $sudo = get('writable_use_sudo') ? 'sudo' : '';

    run("$sudo chown $user:$group -R {{release_path}}");
})->desc('Changing owner and group');
